...
class Server(Thread):
    ...
    def __init__(self):
        ...
        self.__objects = MapGenerator().generate_objects_list()
    ...
    def __init_chunk(self):
        for obj in self.__objects:
            self.__add_to_chunk(obj)

    def __add_to_chunk(self, obj):
        (x, y) = obj.get_position()
        id_x = 0
        id_y = 0

        while id_y < self.__chunks_y:
            if (id_y + 1) * CHUNK_HEIGHT > y:
                break
            id_y += 1

        while id_x < self.__chunks_x:
            if (id_x + 1) * CHUNK_WIDTH > x:
                break
            id_x += 1

        id = int(self.__chunks_x * id_y + id_x)
        # print(f"id {id} id_x {id_x} id_y {id_x}")
        self.__chunks_list[id].add_object(obj)
    ...
    def __create_chunks(self):
        ...
        self.__chunks_x = self.__chunks_list.__len__() / self.__chunks_y
        print("chunk num: " + str(self.__chunks_list.__len__()))
    ...
    def __generate_random_move(self):
        for obj in self.__objects:
            if obj.object_type == ObjectTypes.ENEMY:
                obj.random_move()

    def __move_enemies(self):
        for obj in self.__objects:
            if obj.object_type == ObjectTypes.ENEMY:
                self.__handle_timeout_move(obj)

    def __handle_timeout_move(self, obj):
        if time.time() - 0.1 > self.__last_villain_update:
            self.__last_villain_update = time.time()
            obj.move_object()
            all_characters = self.get_close_characters(obj)
            for character in all_characters:
                client = self.find_client_by_character(character)
                self.__put_package_to_queue((self.__serialize_object(obj), client.get_address()))
